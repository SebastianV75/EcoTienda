-- Save a quotation and its items as one transaction.
-- The application calls this only after its admin role check; this function
-- repeats the authorization check because SECURITY DEFINER bypasses table RLS.
...
begin
  if auth.uid() is null
    ...
    raise exception 'Solo un administrador puede guardar cotizaciones.'
      using errcode = '42501';
  end if;
