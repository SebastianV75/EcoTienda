...
export async function saveVisitaMinisplitAction(
	...
	if (visitaError) {
		return { error: "No se pudo guardar la visita de minisplit.", success: null };
	}
	...
	if (quotationError) {
		console.error("[Visita Minisplit] Error creando cotización automática:", quotationError);
	}
	...
	return { error: null, success: "Visita de minisplit guardada correctamente. Cotización creada automáticamente.", quotationId };
}
