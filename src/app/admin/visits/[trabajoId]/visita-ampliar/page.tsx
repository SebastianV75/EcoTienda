...
import { VisitaAmpliarForm } from "@/features/trabajos/visita-ampliar-form";

export default async function VisitaAmpliarPage({
	params,
}: {
	...
	return (
		<AppShell
			...
			description="Visita Técnica Ampliar Sistema"
			email={user.email}
		>
			<div className="space-y-4">
					...
					<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
						<div className="min-w-0">
							<h1 className="text-2xl font-semibold tracking-[-0.05em] text-[var(--brand-deep)] sm:text-[1.9rem]">
								Visita Técnica Ampliar Sistema
							</h1>
							<p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--muted)]">
								Completa el formulario de visita técnica para ampliar sistema.
							</p>
						</div>
				...
				<section className="rounded-[24px] border border-[var(--border-soft)] bg-white p-5 shadow-sm sm:p-6">
					<VisitaAmpliarForm trabajoId={trabajoId} />
				</section>
			</div>
		</AppShell>
	);
